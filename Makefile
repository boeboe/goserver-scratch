...
build: ## Build the container
	docker build --rm -t $(DOCKER_ACCOUNT)/$(APP_NAME) --build-arg VERSION=${VERSION} .

build-nc: ## Build the container without caching
	docker build --rm --no-cache -t $(DOCKER_ACCOUNT)/$(APP_NAME) --build-arg VERSION=${VERSION} .

run: ## Run container
	docker run -i -t --rm -p 8080:8080 --name="$(APP_NAME)" $(DOCKER_ACCOUNT)/$(APP_NAME)
